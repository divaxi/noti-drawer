...
import (
	...
	"github.com/divaxi/noti-drawer/internal"
)
...
// Logging facilitates logging within Notidrawer. The default log is
...
// that prefix.
//
// Notidrawer logs (except the sink) are zero-allocation, so they are
...
type Logging struct {
	...
	// in Notidrawer. Because it is global and unstructured, the sink
...
func (logging *Logging) openLogs(ctx Context) error {
	...
	// as a special case, set up the default structured Notidrawer log next
...
func (logging *Logging) setupNewDefault(ctx Context) error {
	...
	if userDefault, ok := logging.Logs[DefaultLoggerName]; ok {
		...
		if err != nil {
			return fmt.Errorf("setting up default Notidrawer log: %v", err)
		}
	...
	// redirect the default notidrawer logs
...
func (logging *Logging) Logger(mod Module) *zap.Logger {
	modID := string(mod.NotidrawerModule().ID)
...
type BaseLog struct {
	...
	WriterRaw json.RawMessage `json:"writer,omitempty" notidrawer:"namespace=notidrawer.logging.writers inline_key=output"`

	// The encoder is how the log entries are formatted or encoded.
	EncoderRaw json.RawMessage `json:"encoder,omitempty" notidrawer:"namespace=notidrawer.logging.encoders inline_key=format"`

	// Tees entries through a zap.Core module which can extract
	// log entry metadata and fields for further processing.
	CoreRaw json.RawMessage `json:"core,omitempty" notidrawer:"namespace=notidrawer.logging.cores inline_key=module"`
...
func (cl *BaseLog) buildOptions() ([]zap.Option, error) {
	...
	if cl.WithStacktrace != "" {
		...
		if err != nil {
			return options, fmt.Errorf("setting up default Notidrawer log: %v", err)
		}
...
// Notidrawer will use the standard logger. This is also known as
// the "sink" logger.
type SinkLog struct {
	BaseLog
}
...
// NotidrawerModule returns the Notidrawer module information.
func (StdoutWriter) NotidrawerModule() ModuleInfo {
	return ModuleInfo{
		ID:  "notidrawer.logging.writers.stdout",
		New: func() Module { return new(StdoutWriter) },
	}
}

// NotidrawerModule returns the Notidrawer module information.
func (StderrWriter) NotidrawerModule() ModuleInfo {
	return ModuleInfo{
		ID:  "notidrawer.logging.writers.stderr",
		New: func() Module { return new(StderrWriter) },
	}
}

// NotidrawerModule returns the Notidrawer module information.
func (DiscardWriter) NotidrawerModule() ModuleInfo {
	return ModuleInfo{
		ID:  "notidrawer.logging.writers.discard",
