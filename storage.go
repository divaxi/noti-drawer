...
type Storage interface {
	Store(key string, data []byte) error // Lưu ảnh
	Load(key string) ([]byte, error)     // Đọc ảnh
	Exists(key string) bool              // Kiểm tra xem ảnh đã xử lý chưa (Cache)
}
...
func (s *FileStorage) Exists(_key string) bool {
	return false
}
...
var DefaultStorage = &FileStorage{Path: AppDataDir()}
