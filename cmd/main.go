...
import (
	...
	notidrawer "github.com/divaxi/noti-drawer"
...
// Main implements the main function of the notidrawer command.
// Call this if notidrawer is to be the main() of your program.
func Main() {
	if len(os.Args) == 0 {
...
func LoadConfig(configFile, adapterName string) ([]byte, string, string, error) {
	return loadConfigWithLogger(notidrawer.Log(), configFile, adapterName)
}

func isnotidrawerfile(configFile, adapterName string) (bool, error) {
	if adapterName == "notidrawerfile" {
		return true, nil
	}

	// as a special case, if a config file starts with "notidrawerfile" or
	// has a ".notidrawerfile" extension, and no adapter is specified, and
	// no adapter module name matches the extension, assume
	// notidrawerfile adapter for convenience
	baseConfig := strings.ToLower(filepath.Base(configFile))
	baseConfigExt := filepath.Ext(baseConfig)
	startsOrEndsInnotidrawerfile := strings.HasPrefix(baseConfig, "caddyfile") || strings.HasSuffix(baseConfig, ".caddyfile")
	...
	// the config file starts with "notidrawerfile",
	// the config file has an extension,
	// and isn't a JSON file (e.g. notidrawerfile.yaml),
	// then we don't know what the config format is.
	if adapterName == "" && startsOrEndsInnotidrawerfile {
		return true, nil
	}

	// adapter is not empty,
	// adapter is not "notidrawerfile",
	// extension is not ".json",
	// extension is not ".notidrawerfile"
	// file does not start with "notidrawerfile"
	return false, nil
}
...
func loadConfigWithLogger(logger *zap.Logger, configFile, adapterName string) ([]byte, string, string, error) {
	...
	var config []byte
	var cfgAdapter notidrawerconfig.Adapter
	var err error
	if configFile != "" {
		if configFile == "-" {
		...
		// if the notidrawerfile adapter is plugged in, we can try using an
		// adjacent notidrawerfile by default
		cfgAdapter = notidrawerconfig.GetAdapter("caddyfile")
		if cfgAdapter != nil {
			config, err = os.ReadFile("notidrawerfile")
			if errors.Is(err, fs.ErrNotExist) {
				// okay, no default notidrawerfile; pretend like this never happened
				cfgAdapter = nil
			} else if err != nil {
				// default notidrawerfile exists, but error reading it
				return nil, "", "", fmt.Errorf("reading default notidrawerfile: %v", err)
			} else {
				// success reading default notidrawerfile
				configFile = "notidrawerfile"
				logger.Info("using adjacent notidrawerfile")
			}
		}
	}

	if yes, err := isnotidrawerfile(configFile, adapterName); yes {
		adapterName = "notidrawerfile"
	} else if err != nil {
		return nil, "", "", err
	}
	...
	// load config adapter
	if adapterName != "" {
		cfgAdapter = notidrawerconfig.GetAdapter(adapterName)
		if cfgAdapter == nil {
...
func watchConfigFile(filename, adapterName string) {
	...
	for range time.Tick(1 * time.Second) {
		...
		err = notidrawer.Load(lastCfg, false)
		if err != nil {
...
func (f Flags) Duration(name string) time.Duration {
	val, _ := notidrawer.ParseDuration(f.String(name))
	return val
}
...
func loadEnvFromFile(envFile string) error {
	...
	notidrawer.ConfigAutosavePath = filepath.Join(notidrawer.AppConfigDir(), "autosave.json")
	notidrawer.DefaultStorage = &certmagic.FileStorage{Path: notidrawer.AppDataDir()}
...
func printEnvironment() {
	_, version := notidrawer.Version()
	fmt.Printf("notidrawer.HomeDir=%s\n", notidrawer.HomeDir())
	fmt.Printf("notidrawer.AppDataDir=%s\n", notidrawer.AppDataDir())
	fmt.Printf("notidrawer.AppConfigDir=%s\n", notidrawer.AppConfigDir())
	fmt.Printf("notidrawer.ConfigAutosavePath=%s\n", notidrawer.ConfigAutosavePath)
	fmt.Printf("notidrawer.Version=%s\n", version)
