...
// Module is a type that is used as a Notidrawer module. In
...
// type-asserted as notidrawerhttp.MiddlewareHandler values.
// 6) When a module's containing Context is canceled, if it is
// a CleanerUpper, its Cleanup() method is called.
type Module interface {
	// This method indicates that the type is a Notidrawer
	// module. The returned ModuleInfo must have both
	// a name and a constructor function. This method
	// must not have any side-effects.
	NotidrawerModule() ModuleInfo
}

// ModuleInfo represents a registered Notidrawer module.
...
// ModuleID is a string that uniquely identifies a Notidrawer module. A
...
// for app modules (these are "top-level" modules that Notidrawer core
// loads and runs).
//
...
// - notidrawer.logging.encoders.json
...
func RegisterModule(instance Module) {
	mod := instance.NotidrawerModule()

	if mod.ID == "" {
		panic("module ID missing")
	}
	if mod.ID == "notidrawer" || mod.ID == "admin" {
...
func GetModuleName(instance any) string {
	...
	if mod, ok := instance.(Module); ok {
		name = mod.NotidrawerModule().ID.Name()
	}
	return name
}
...
func GetModuleID(instance any) string {
	var id string
	if mod, ok := instance.(Module); ok {
		id = string(mod.NotidrawerModule().ID)
	}
	return id
}
...
// ParseStructTag parses a notidrawer struct tag into its keys and values.
// It is very simple. The expected syntax is:
// `notidrawer:"key1=val1 key2=val2 ..."`
...
// in the namespace `notidrawer.network_proxy` must implement this
// interface.
